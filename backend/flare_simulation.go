...
func main() {
	...
	for i := 0; i < len(race_track.Segments); i++ {
		if ((i+1) == len(race_track.Segments)-1){
			break
		}
		speed := 50.0
		...
		if (upcomingCurve.Radius > 0) {
			...
			if (DistanceToCoast > race_track.Segments[i].Length){
				fmt.Println("NOT ENOUGH TRACK TO SLOW DOWN. BRAKE!!", "I NEED ", DistanceToCoast)
			}
			fmt.Println("IM SLOWING IT DOWN TWIN", "I NEED ", DistanceToCoast)
		}
	}
